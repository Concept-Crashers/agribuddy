...
const sendSMS = async (to, message) => {
    try {
        // Africa's Talking requires strict E.164 format. 
        // We auto-convert local Ugandan 07... numbers to +2567...
        let formattedTo = Array.isArray(to) ? to.join(',') : to;
        if (formattedTo.startsWith('0')) {
            formattedTo = '+256' + formattedTo.slice(1);
        ...
        // We MUST throw the error so the controller catch blocks can intercept it and alert the frontend!
        throw new Error(error.response?.data?.Message || 'Failed to send SMS via Africa\'s Talking');
    }
};
